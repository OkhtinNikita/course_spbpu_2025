...
def __integr_func(
    ...
    # Полный градиент давления (гравитация + трение)
...
def calc_pipe(
    ...
    if not solution.success:
        raise RuntimeError(f'Интегрирование не выполнено')
...
gamma_water = 1.0368694338193978 # относительная плотность по пресной воде с плотностью 1000 кг/м3, безразм
H = 3103.9385524723707 # измеренная глубина забоя скважины
d_tub = 0.0803241605369353 # диаметр НКТ, м
angle = 60.09524949631164 # угол наклона скважины к горизонтали, градусы
roughness = 0.0005694032496649107 # шероховатость трубы, м
p_wh = 169.62629023200577 * 101325 # давление на устье, атм
t_wh = 25.10705416792023 # температура на устье скважины, С
temp_grad = 2.8305633946722457 # геотермический градиент, К/м * (1e-2)
h0 = 0
...
# Диапазон дебита жидкости, м3/сут
q_range = np.linspace(1, 400, 50)

# Зададим список значения забойного давления
p_wf_range = []
...
# Формирование итогового JSON output.json
...
print(json.dumps(vlp_result, indent=4))

with open('output.json', 'w', encoding='utf-8') as f:
    json.dump(vlp_result, f, indent=4, ensure_ascii=False)
